...
public class MyActivity extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    findViewById(R.id.oneHelper)
        .setOnClickListener(view -> {
          allCount.set(0);
          final List<DbInsertThread> allThreads = new ArrayList<>();
    ...
    findViewById(R.id.testTransactionIsolation).setOnClickListener(view -> {
      DatabaseHelper helper = new DatabaseHelper(MyActivity.this);
      new SlowInsertThread(helper).start();
      new FastSelectThread(helper).start();
    });

    findViewById(R.id.noTrans).setOnClickListener(view -> runNoTrans());

    findViewById(R.id.trans).setOnClickListener(view -> runTrans());
  }
  ...
  private void showTime(long start, DatabaseHelper instance) {
    ...
    ((TextView) findViewById(R.id.timeOut)).setText(
        "Total rows: " + instance.countSessions() + "/time: " + Long.toString(time));
  }

  private void insertTest(DatabaseHelper helper) {

    int numberOfInserts = Integer.parseInt(
        ((EditText) findViewById(R.id.numberOfInserts)).getText().toString());
  ...
  static class SlowInsertThread extends Thread {
  ...
  static class FastSelectThread extends Thread {
    ...
    new Thread(new Runnable() {
      public void run() {
        ...
        uiHandler.post(new Runnable() {
          public void run() {
            ((TextView) findViewById(R.id.results)).setText("All thread Inserted " + allCount.get());
          }
        });
      }
    }).start();
